...
export default function InsightHubPlanos() {
  return (
    <div className="space-y-8">
      <header>
        <h1 className="font-display text-2xl font-bold">Planos Insight Hub</h1>
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
          Valores na mesma ordem de grandeza das referências de mercado (mLabs / Reportei), com identidade AD-Hub. A ativação
          grava <code className="rounded bg-muted px-1 text-xs">insight_hub_entitlements</code> por organização — dados isolados
          por <code className="rounded bg-muted px-1 text-xs">organization_id</code>.
        </p>
      </header>
      ...
      <Card className="border-dashed bg-muted/20">
        <CardHeader>
          <CardTitle className="text-base">Nota técnica</CardTitle>
          <CardDescription>
            Após o pagamento confirmado, insira ou atualize a linha em{" "}
            <code className="text-xs">insight_hub_entitlements</code> (ver{" "}
            <code className="text-xs">database/mysql/006_insight_hub.sql</code>) e inclua o módulo{" "}
            <code className="text-xs">insight-hub</code> no JSON <code className="text-xs">enabled_modules</code> da
            organização, se não usar o modo “todos os módulos”.
          </CardDescription>
        </CardHeader>
      </Card>
    </div>
